 ...
 *
 * Default backend is NativeAccountStore (atomic JSON file). Set
 * ATL_ACCOUNT_BACKEND=remote to use RemoteAccountStore (OrganicOS helper IPC;
 * follow-up upstream MR).
 *
 * GMS / Google Sign-In are intentionally out of scope for this slice.
 */
public class AccountManager {
	...
	private static AccountStore createStore() {
		String backend = System.getenv("ATL_ACCOUNT_BACKEND");
		if (backend != null && backend.equalsIgnoreCase("remote")) {
			return new RemoteAccountStore();
		}
